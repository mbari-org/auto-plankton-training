...
class EarlyStopping:
    ...
    def __call__(self, val_acc, model):
        ...
        """    
        score = val_acc  # Using negative loss to treat lower losses as better scores.

        if self.best_score is None:
            # Save the model at the first run.
            self.best_score = score
            self.save_checkpoint(val_acc, model, first_run=True)
        elif score < self.best_score + self.delta:
            # No improvement in validation loss.
            self.counter += 1
            if self.verbose:
                print(f'EarlyStopping counter: {self.counter} out of {self.patience}  ({val_acc:.6f} is worse than {self.best_score:.6f})\n')
